...
```{r}
esLong = seq(350,2500)
weightsEs = ifelse(esLong > 1050, 4, 1.4)
...
```{r}
#Nitrogen p values
ggplot(df_result,aes(x=espectro))+
  geom_point(aes(y=-log(nitro_LiC, base = 10),color='black'),size=1,shape=16)+
  geom_point(aes(y=-log(nitro_LiCW, base = 10),color='red'),size=1,shape=16)+
  geom_hline(yintercept = -log(0.05, base = 10), color = "darkblue",linetype='dashed') +
  geom_hline(yintercept = -log(0.03, base = 10), color = "red",linetype='dashed') +
  geom_hline(yintercept = -log(0.01, base = 10), color = "black",linetype='dashed') +
