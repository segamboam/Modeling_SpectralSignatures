...
ggplot(df_graph,aes(x=x))+
  ...
  geom_point(aes(y=-log(nitro_0, base = 10),color='red'),size=1,shape=16)+
  geom_hline(yintercept = -log(0.05, base = 10), color = "darkblue",linetype='dashed') +
  theme_minimal()+
  scale_color_manual(values = c("black",'orange'), labels = c( "Normal","Corregido")) +
  xlab('Wavelength (nm)')+
...
```{r}
esLong = seq(350,2500)
weightsEs = ifelse(esLong > 1050, 2, 1.4)
...
## Visualization comparison resultados
